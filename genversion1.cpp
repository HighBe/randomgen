...
string assemblyfor(int depth);
string assemblyif(int depth);
string assemblyswitch(int depth);
string assemblystate(int depth);
...
string inlineassembly(){
    ...
    */
    ...
    for(int i = 0;i < 3;++ i){//内联汇编中有3个基本的for或if块
        switch(rand() % 2){
        case 0:
            assemblyfor(0);
            break;
...
string assemblyfor(int depth){ // i控制深度,tmp表示有多少个变量
    if(depth >3)return "";
    string var;
    var.push_back('i'+depth);
    string res = "for {let "+var+" := 0} lt("+var+"," + to_string(rand() % 10) + ") {"+var+" := add(i,1)}\n{\n";
    // 添加赋值语句
    // x := y  x := add(y,z)  x := mul(y,z)
    int tmpi = rand() % 5 + 5;
    ...
    switch(rand() % 4){
    case 0:
        res += assemblyfor(depth+1);
        break;
    case 1:
        res += assemblyif(depth+1);
        break;
    case 2:
        res += "break\n";
        break;
...
string assemblyswitch(int depth){
    ...
    switch(rand() % 2){
    case 0:
        res += "addmod(" + AssemblyVarList[rand() % AssemblyVarList.size()].Variablename + "," + AssemblyVarList[rand()%AssemblyVarList.size()].Variablename + "," + "2)";
        break;
    case 1:
        res += "mulmod(" + AssemblyVarList[rand() % AssemblyVarList.size()].Variablename + "," + AssemblyVarList[rand()%AssemblyVarList.size()].Variablename + "," + "2)";
        break;
    }
    res += ")\ncase 0:{\n"; //每种case可以加入for if switch state四种情况
    switch (rand() % 4)
    {
    case 0:
        res += assemblyfor(depth+1);
    ...
    case 1:
        ...
        break;
    case 2:
        res += assemblyswitch(depth+1);
        break;
    case 3:
        res += assemblystate(depth+1);
        break;
    }
    res += "}\ncase 1:{\n";
    switch (rand() % 4)
...
string assemblystate(int depth){
    ...
    int tmpl = rand() % 5 + 5;
    if(depth == 0){
        res += AssemblyVarList[rand() % AssemblyVarList.size()].Variablename + " := ";
    }
    if(depth >= 3){
        res += AssemblyVarList[rand() % AssemblyVarList.size()].Variablename;
        return res;
    ...
    for(int i = 0;i < tmpl;++ i){
        switch(rand() % 8){
        case 0:
            res += "add(" + assemblystate(depth+1) + "," + assemblystate(depth+1) + ")";
            break;
        case 1:
            res += "mul(" + assemblystate(depth+1) + "," + assemblystate(depth+1) + ")";
            break;
        case 2:
            res += assemblystate(depth+1);
            break;
        case 3:
            res += "addmod(" + assemblystate(depth+1) + "," + assemblystate(depth+1)+"," + assemblystate(depth+1)+")";
            break;
        case 4:
            res += "mulmod(" + assemblystate(depth+1) + "," + assemblystate(depth+1)+"," + assemblystate(depth+1)+")";
            break;
        case 5:
            res += "div(" + assemblystate(depth+1) + "," + assemblystate(depth+1) + ")";
            break;
        case 6:
            res += "mod(" + assemblystate(depth+1) + "," + assemblystate(depth+1) + ")";
            break;
        case 7:
            res += "exp(" + assemblystate(depth+1) + "," + assemblystate(depth+1) + ")";
            break;
        }
    }
